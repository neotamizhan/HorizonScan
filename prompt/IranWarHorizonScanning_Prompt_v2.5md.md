...
## TASK
Compile a structured daily digest covering the current conflict situation as it relates to Kuwait and the Gulf region. Today's date is {INSERT_DATE}. Search for information from the past 24 hours only.
...
Provide the article title, author, publication, date, and a 2-sentence summary. Include the direct URL.

## OUTPUT FORMAT

### 🛡️ Horizon Scanning Report — {INSERT_DATE}
...
### ⚠️ SECTION 5: RISK SIGNAL FOR FINANCIAL INSTITUTIONS
...
### 🔐 SECTION 7: CYBER & THREAT INTELLIGENCE SIGNALS
...
**Sources to search (public-access only):**

*Official / Government CERTs:*
- CISA (cisa.gov/news-events/alerts) — US but globally referenced
- Kuwait e-Government CIRT (cirt.gov.kw)
- UAE Cybersecurity Council (cybersecurity.ae)
- Saudi NCSC (ncsc.gov.sa)
- NCSC UK (ncsc.gov.uk/news) — for Gulf-relevant advisories
- ENISA (enisa.europa.eu/news) — for financial sector alerts

*Commercial Threat Intelligence (public blog/bulletin outputs only):*
- Recorded Future public intelligence briefs (recordedfuture.com/blog)
- Mandiant Threat Intelligence blog (cloud.google.com/blog/topics/threat-intelligence)
- CrowdStrike Adversary Intelligence blog (crowdstrike.com/blog)
- Flashpoint public reports (flashpoint.io/blog)
- Dragos (dragos.com/blog) — for critical infrastructure and OT threats
- Secureworks CTU blog (secureworks.com/blog)

*OSINT & Community:*
- Bellingcat (for conflict-linked cyber activity only)
- The Shadowserver Foundation (shadowserver.org/news)
- SANS Internet Stormcast (isc.sans.edu) — for active exploitation signals
...
**Relevance filter — only include if the item relates to one or more of:**
...
If no relevant advisories are found: state "No Kuwait or Gulf-relevant cyber advisories identified in the past 24–48 hours from monitored public sources."
